...
import { getCOPCReport } from '../api/client'
...
export default function COPCReport() {
  ...
  async function load() {
    setLoading(true)
    try {
      const data = await getCOPCReport()
            ...
            <div key={emp.user_id}>
              ...
              <div onClick={() => setExpanded(expanded === emp.user_id ? null : emp.user_id)}
                ...
                <div style={{ textAlign: 'center', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
                  {emp.green_count > 0 && <span style={{ background: 'var(--k-success-bg)', color: 'var(--k-success-text)', padding: '2px 7px', borderRadius: '20px', fontSize: '11px', fontWeight: 700 }}>✓{emp.green_count}</span>}
                  {emp.amber_count > 0 && <span style={{ background: 'var(--k-warning-bg)', color: 'var(--k-warning-text)', padding: '2px 7px', borderRadius: '20px', fontSize: '11px', fontWeight: 700 }}>~{emp.amber_count}</span>}
                  {emp.red_count > 0 && <span style={{ background: 'var(--k-danger-bg)', color: 'var(--k-danger-text)', padding: '2px 7px', borderRadius: '20px', fontSize: '11px', fontWeight: 700 }}>✕{emp.red_count}</span>}
                </div>
              </div>
